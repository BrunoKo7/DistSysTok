	...
	public void run() {
		...
		try {
			...
			if (requestType.equals("NV")) {
				...
				System.out.println(">Publisher->" + this.channelName.getChannelName() + ": Messages to send:");

				for (VideoFileChunk vfc : vfcl) {
					System.out.println(">Publisher " + vfc.getorder());
				...
				System.out.println(">Publisher has videos:");
				...
				this.objOS.flush();
				System.out.println(">Publisher sends number of videos: " + videos.size());
				...
				for (String videoName : videos) {
					...
					for (VideoFileChunk vfc : chunks) {
						System.out.println(">Publisher sends video with orderID " + vfc.getorder());
						this.objOS.writeObject(vfc);
						this.objOS.flush();
					}
				}
